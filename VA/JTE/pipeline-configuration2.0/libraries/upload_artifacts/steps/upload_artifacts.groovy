...
if (agentname.contains(oswindows)) {
         ...
         rtUpload (
          ...
          spec: '''{
              "files": [
                {
                  "pattern": "test.zip",
                  "target": "libs-release-local//Release-Dev/"
                }
            ]
        }''', 
        buildName: JOB_NAME,
    ...
    spec: '''{
          "files": [
            {
              "pattern": "uploadartifactsflag.txt",
              "target": "libs-release-local/Release-Dev/"
            }
         ]
    }''',
...
if (agentname.contains(oslinux)){      
    ...
    spec: '''{
          "files": [
            {
              "pattern": "target/libmodel-0.0.1-SNAPSHOT.jar",
              "target": "libs-snapshot//0.0.1-SNAPSHOT/"
            },
	     {
      		"pattern": "pom.xml",
      		"target": "libs-snapshot/gov/libmodel-0.0.1-SNAPSHOT.pom"
    	     }
         ]
    }''',
